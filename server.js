        ...
        try {
          ...
          if (closed) {
            ...
            if (tradeIdx !== -1) {
              ...
              data.trades[tradeIdx].exitDate  = new Date(closeDate).toISOString();
...
// ── Info (équivalent /status de NEXUS STOCKS) ───────────────────
app.get('/', (_, res) => {
  res.json({
    status:   'running',
    engine:   'NEXUS CRYPTO',
    ...
    tpPct:    TP_PCT * 100 + '%',
    signal: {
      buy:  'macdCrossUp + ema50_200Bear + aroonBear + atrHigh + momentumAccelBear — WR 63% · WF-moy 69% · PF 2.78',
      sell: 'mfiOversold + accDistBear + atrNormal + cciOverbought + higherHighs — WR 62% · WF-moy 69% · PF 2.72'
    }
  });
});
