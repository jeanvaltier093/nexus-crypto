...
const BUY_COMBO  = ['macdDivBear', 'obvFalling', 'cmfBear', 'atrRising', 'insideBarBear'];
const SELL_COMBO = ['rsiOverbought', 'atrNormal', 'lowerLows', 'lowLiquidityHour', 'momentumAccelBull'];
...
function detectSignals(candles) {
  ...
  // ── BUY Rang 1: macdDivBear + obvFalling + cmfBear + atrRising + insideBarBear ──
 
  // macdDivBear: prix monte mais MACD ligne descend (divergence baissière)
  if (MACD && MACD.line.length >= 4) {
    const ml = MACD.line;
    const priceTrendUp = C[n] > C[n-3];
    const macdTrendDown = ml[ml.length-1] < ml[ml.length-4];
    s.macdDivBear = priceTrendUp && macdTrendDown;
  } else { s.macdDivBear = false; }
 
  // obvFalling: OBV en baisse sur 10 bougies
  if (n >= 10) {
    let obv = 0, obvPrev = 0;
    for (let i = 1; i <= n; i++)       obv  += C[i] > C[i-1] ? V[i] : C[i] < C[i-1] ? -V[i] : 0;
    for (let i = 1; i <= n-10; i++)    obvPrev += C[i] > C[i-1] ? V[i] : C[i] < C[i-1] ? -V[i] : 0;
    s.obvFalling = obv < obvPrev;
  } else { s.obvFalling = false; }
 
  // cmfBear: Chaikin Money Flow < 0 sur 20 bougies
  if (n >= 20) {
    let mfvSum = 0, volSum = 0;
    for (let i = n-19; i <= n; i++) {
      const hl = H[i] - L[i];
      if (hl > 0) mfvSum += ((C[i]-L[i]) - (H[i]-C[i])) / hl * V[i];
      volSum += V[i];
    }
    s.cmfBear = volSum > 0 && (mfvSum / volSum) < 0;
  } else { s.cmfBear = false; }
 
  // atrRising: ATR actuel > ATR il y a 5 bougies
  if (ATR && ATR.length >= 6) {
    s.atrRising = ATR[ATR.length-1] > ATR[ATR.length-6];
  } else { s.atrRising = false; }
 
  // insideBarBear: inside bar + clôture baissière
  s.insideBarBear = n >= 1 && H[n] < H[n-1] && L[n] > L[n-1] && C[n] < candles[n].open;
 
  // ── SELL Rang 21: rsiOverbought + atrNormal + lowerLows + lowLiquidityHour + momentumAccelBull ──
 
  // rsiOverbought: RSI > 70
  if (MFI) { // MFI shares RSI computation path; compute RSI separately
    const rsiPeriod = 14;
    if (C.length > rsiPeriod) {
      ...
      s.rsiOverbought = (100 - 100 / (1 + rs)) > 70;
    } else { s.rsiOverbought = false; }
  } else { s.rsiOverbought = false; }
 
  // atrNormal: ATR dans une plage calme (0.5x à 1.2x de la moyenne)
  ...
  // lowerLows: 3 bas consécutifs décroissants
  s.lowerLows = n >= 2 && L[n] < L[n-1] && L[n-1] < L[n-2];
 
  // lowLiquidityHour: volume actuel < 70% de la moyenne sur 20 bougies
  if (n >= 20) {
    const avgVol = V.slice(n-20, n).reduce((a,b)=>a+b,0) / 20;
    s.lowLiquidityHour = avgVol > 0 && V[n] < avgVol * 0.7;
  } else { s.lowLiquidityHour = false; }
 
  // momentumAccelBull: momentum accélérant à la hausse sur 3 bougies
  if (MOM && MOM.length >= 3) {
    const l = MOM.length;
    s.momentumAccelBull = MOM[l-1] > MOM[l-2] && MOM[l-2] > MOM[l-3];
  } else { s.momentumAccelBull = false; }
...
app.get('/', (_, res) => {
  res.json({ status:'running', engine:'NEXUS CRYPTO', version:'1.0.0', time:new Date().toISOString(), scanning:scanRunning, cryptos:CRYPTOS, slPct:SL_PCT*100+'%', tpPct:TP_PCT*100+'%', signal:{ buy:'macdDivBear + obvFalling + cmfBear + atrRising + insideBarBear — WR 66% · WF-moy 73% · PF 3.21', sell:'rsiOverbought + atrNormal + lowerLows + lowLiquidityHour + momentumAccelBull — WR 61% · WF-moy 65% · PF 2.64' } });
});
